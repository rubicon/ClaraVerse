...
export interface FloatingIconsHeroProps {
  ...
  icons: IconProps[];
  onCtaClick?: () => void;
  compact?: boolean; // For split-screen layouts
}
...
>(
  (
    { className, title, subtitle, ctaText, ctaHref, icons, onCtaClick, compact = false, ...props },
    ref
  ) => {
    ...
    return (
      <section
        ...
        className={cn(
          ...
          compact ? 'h-full min-h-0' : 'h-screen min-h-[700px]',
          className
        )}
        {...props}
      >
        ...
        <div
          className={cn(
            'relative z-10 text-center px-4 mx-auto',
            compact ? 'space-y-4 max-w-xl' : 'space-y-8 max-w-4xl'
          )}
        >
          <Typography variant={compact ? 'h1' : 'display'} weight="bold" align="center" gradient>
            {title}
          </Typography>
          <Typography
            variant={compact ? 'base' : 'lg'}
            align="center"
            className={cn('mx-auto text-muted-foreground', compact ? 'max-w-md' : 'max-w-2xl')}
          >
            {subtitle}
          </Typography>
          {ctaText && (
